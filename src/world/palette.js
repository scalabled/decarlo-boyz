...
export const PALETTE = {
  ...
  /* ==================================================================== */
  /* STEEL CITY — the open-world surfaces                                 */
  /*                                                                      */
  /* These are thin aliases onto the road/nature families `materials`      */
  /* provides for the open world. Nothing here re-authors a surface —      */
  /* the palette's job is to name the small, deliberate set Steel City is  */
  /* allowed to use, so every mesh sharing a key merges into one draw.     */
  /* ==================================================================== */

  /**
   * The carriageway on a straight. LANE-ALIGNED: the mesh carries u across the
   * road and v along it, so the surface's wheel-polish bands, the oil line down
   * the lane centre and the paver's cold joint all land where traffic actually
   * put them. `roadmesh` maps 1 uv unit = 4 m, which is what the library asks
   * for. This is the single biggest signal on a road that a stretched grey
   * plane does not have.
   */
  road: { name: 'road_lane', surface: 'asphalt', opts: { tint: 0x6b6b68 } },
  /** Junction pads: no lane direction exists there, so world projection. */
  road_pad: { name: 'road_asphalt', surface: 'asphalt', opts: { tint: 0x6b6b68 } },
  /** Back streets and mill approaches: cut, patched and cracked to pieces. */
  road_patch: { name: 'road_asphalt_patched', surface: 'asphalt', opts: { tint: 0x5c5c57 } },

  walk: { name: 'sidewalk', surface: 'sidewalk', opts: {} },
  walk_old: { name: 'sidewalk_old', surface: 'sidewalk', opts: {} },
  kerb: { name: 'kerb', surface: 'sidewalk', opts: {} },

  /**
   * Markings are alpha-masked decals whose glyph, wear and junction scrub are
   * all in the surface, so `roadmesh` emits ONE quad per line and the material
   * dashes it, thins it and rubs it through at the wheel line.
   */
  line_white: { name: 'road_line', surface: 'asphalt', opts: {} },
  line_dash: { name: 'road_line_dash', surface: 'asphalt', opts: {} },
  line_yellow: { name: 'road_line_double', surface: 'asphalt', opts: {} },
  mark_cross: { name: 'road_crossing', surface: 'asphalt', opts: {} },
  mark_stop: { name: 'road_stopbar', surface: 'asphalt', opts: {} },

  drain: { name: 'drain_grate', surface: 'metal', opts: {} },
  cover: { name: 'manhole', surface: 'metal', opts: {} },

  /* ---------------------------------------------------------- terrain --- */
  /** Valley floor and hillside. */
  terrain_grass: { name: 'grass', surface: 'grass', opts: { tint: 0xbfc6ac } },
  /** Flats, mill land, and everything the rain has churned. */
  terrain_dirt: { name: 'mud', surface: 'dirt', opts: {} },
  /** Anything steeper than ~32 degrees: the Mt. Washington face, cut banks. */
  terrain_rock: { name: 'gravel', surface: 'gravel', opts: { uvMode: 'triplanar', scale: 2.6, tint: 0x8b877e, macroRelief: 0.9 } },
  /** The river bank between the waterline and the top of the bank. */
  terrain_silt: { name: 'river_silt', surface: 'dirt', opts: {} },
  /** The strip between the back of the pavement and undisturbed ground. */
  verge: { name: 'grass_verge', surface: 'grass', opts: { tint: 0xbcc2a8 } },
  /**
   * The RISER of that strip.
   *
   * `netgen.rasteriseRoads` deliberately sinks the ground half a metre under
   * every corridor so it can never come up through the tarmac, and the kerb and
   * pavement stand another 15 cm above that. Behind an urban pavement the verge
   * therefore has 0.7 m to lose across less than a metre of ground — a face at
   * 40-70 degrees. Grass does not grow on a 70-degree face, and drawing it with
   * `grass_verge` is what a critic panel reported as "the kerb's vertical face
   * has grass on it". The step is a low retaining wall and it is poured
   * concrete, projected triplanar so a near-vertical band does not smear.
   */
  verge_riser: {
    name: 'concrete',
    surface: 'concrete',
    opts: { uvMode: 'triplanar', scale: 1.9, tint: 0x8e8a80, macroRelief: 0.45 },
  },

  /* ------------------------------------------------------------- rail --- */
  ballast: { name: 'gravel', surface: 'gravel', opts: { uvMode: 'triplanar', scale: 1.15, tint: 0x8b8478 } },
  rail_steel: { name: 'plate_steel', surface: 'metal', opts: { vertexMasks: true } },
  sleeper: {
    name: 'wood',
    surface: 'wood',
  ...
  bridge_steel: { name: 'bridge_steel', surface: 'metal', opts: { vertexMasks: true } },
  bridge_rail: { name: 'plate_steel', surface: 'metal', opts: { vertexMasks: true } },
};
