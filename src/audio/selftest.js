...
import { jingle, JINGLE_IDS } from './jingles.js';
...
import { CITY_EVENTS, cityOneShot } from './city.js';
import { EngineVoice, ENGINE_PROFILES } from './engine.js';
...
export async function runAudioSelfTest(opts = {}) {
  ...
  /* ---- foley ----------------------------------------------------- */
  // The full ARCHITECTURE.md surface vocabulary. Anything missing here falls
  // through to `concrete`, which is exactly the bug that made every footstep in
  // the city sound identical.
  const surfaces = ['concrete', 'asphalt', 'sidewalk', 'metal', 'wood', 'dirt', 'sand', 'grass',
    'gravel', 'glass', 'water', 'foliage', 'fabric', 'flesh', 'rubber', 'plaster', 'carpaint'];
  ...
  /* ---- gameplay jingles ------------------------------------------ */
  // Every authored id in jingles.js, plus the spend variant of the register.
  // Silence here means an id fell out of the table — which is exactly how the
  // reference's distinct jingles collapsed into one generic blip last time.
  for (const k of JINGLE_IDS) {
    await push(`jingle:${k}`, 2.6, ({ bank, rng, mixer, t }) => {
      route(mixer, jingle(mixer.actx, bank, rng, k, { when: t }), 'ui');
    }, { reverb: false });
  }
  await push('jingle:cash:spend', 1.6, ({ bank, rng, mixer, t }) => {
    route(mixer, jingle(mixer.actx, bank, rng, 'cash', { when: t, spend: true }), 'ui');
  }, { reverb: false });
  ...
  /* ---- city one-shots -------------------------------------------- */
  const cityKinds = [...new Set(Object.values(CITY_EVENTS).flat()), 'thunder'];
  for (const k of cityKinds) {
    await push(`city:${k}`, 12, ({ bank, rng, mixer, t }) => {
      route(mixer, cityOneShot(mixer.actx, bank, rng, k, { when: t, near: 0.8 }), 'ambience');
    });
  }

  /* ---- engines ---------------------------------------------------- */
  // The engine is continuous, so unlike every other voice here it has to be
  // TICKED while the offline render is suspended. Rather than duplicate that
  // machinery (see bench.js, which does it properly and measures the result),
  // this is a cheap smoke test: build one of each class, set one steady state,
  // and prove it makes finite, audible, non-clipping sound.
  for (const kind of Object.keys(ENGINE_PROFILES)) {
    await push(`engine:${kind}`, 2, ({ bank, rng, mixer }) => {
      const p = ENGINE_PROFILES[kind];
      const v = new EngineVoice(mixer.actx, bank, rng.fork(), p, true);
      v.out.connect(mixer.bus('vehicles'));
      v.start(1);
      v.setState({
        rpm: p.idle + (p.redline - p.idle) * 0.55, throttle: 0.8, gear: 3, load: 0.8,
        speed: 24, redline: p.redline, idle: p.idle,
      }, 1 / 60, 1);
    }, { reverb: false });
  }
