...
/**
 * `exterior: true` means the bus is heard THROUGH the car body when the player
 * is inside one, so it is summed into the cabin filter rather than straight
 * into the world. The player's own engine, the radio and dialogue are inside
 * the cabin with them and get their own colouring instead.
 */
const BUS_DEFS = {
  weapons:  { trim: 0.95, exterior: true, comp: { threshold: -7, knee: 8, ratio: 2.6, attack: 0.003, release: 0.16 } },
  foley:    { trim: 0.9, exterior: true, comp: { threshold: -14, knee: 10, ratio: 2.0, attack: 0.004, release: 0.2 } },
  ambience: { trim: 0.5, exterior: true, comp: { threshold: -24, knee: 12, ratio: 2.0, attack: 0.05, release: 0.5 } },
  sirens:   { trim: 0.8, exterior: true, comp: { threshold: -16, knee: 10, ratio: 2.4, attack: 0.01, release: 0.3 } },
  // The engine bus carries a lot of sustained low end; it is compressed harder
  // than anything else so a redline pass does not swallow the rest of the mix.
  vehicles: { trim: 0.72, exterior: false, comp: { threshold: -18, knee: 10, ratio: 3.2, attack: 0.02, release: 0.28 } },
  music:    { trim: 0.62, exterior: false, comp: { threshold: -15, knee: 12, ratio: 2.6, attack: 0.012, release: 0.3 } },
  voice:    { trim: 0.85, exterior: false, comp: { threshold: -18, knee: 8, ratio: 3.0, attack: 0.006, release: 0.22 } },
  ui:       { trim: 1.6, exterior: false, comp: null },
};
...
export class Mixer {
  ...
  constructor(actx, rng, opts = {}) {
    ...
    //
    // 0.32 is measured, not chosen: the `mixstress` bench scene — a redlining
    // V8 in the cabin, the radio, six traffic vehicles, three sirens, rain,
    // automatic fire and an explosion — peaks at about -2 dBFS here, so the
    // loudest thing the game can produce still leaves 2 dB before the limiter
    // has to do anything, and a footstep at -50 dBFS is still 50 dB of range
    // below it.
    this.preGain = gain(actx, 0.32);
    ...
    /* ---- the cabin ------------------------------------------------- */
    // Sitting in a car is the single biggest EQ move in an open-world mix: the
    // street goes away, the glass rolls off everything above ~1 kHz and the
    // body panels take 12 dB out of the top. `setCabin(1)` is windows up.
    this.exteriorSum = gain(actx, 1);
    this.cabinLP = biquad(actx, 'lowpass', 20000, 0.6);
    this.cabinHS = biquad(actx, 'highshelf', 2200, 0.7, 0);
    this.cabinGain = gain(actx, 1);
    this.exteriorSum.connect(this.cabinLP);
    this.cabinLP.connect(this.cabinHS);
    this.cabinHS.connect(this.cabinGain);
    this.cabinGain.connect(this.worldSum);
    this.cabin = 0;
  ...
  /**
   * How enclosed the listener is by a vehicle body. 0 = on foot, 1 = sealed
   * cabin. Ramped over ~0.25 s so a door closing is a move, not a switch.
   */
  setCabin(amount, smooth = 0.25) {
    const a = clamp(amount, 0, 1);
    this.cabin = a;
    const t = this.actx.currentTime;
    this.cabinLP.frequency.setTargetAtTime(20000 * Math.pow(0.055, a), t, smooth);
    this.cabinHS.gain.setTargetAtTime(-15 * a, t, smooth);
    this.cabinGain.gain.setTargetAtTime(1 - 0.42 * a, t, smooth);
  }

  /** Duck one named bus. Used for radio-under-siren and radio-under-dialogue. */
  duckBus(name, amount, hold = 0.2) {
    const b = this.buses[name];
    if (!b) return;
    const a = clamp(amount, 0, 0.95);
    if (a <= b.duckAmount + 0.01) return;
    const t = this.actx.currentTime;
    b.duckAmount = a;
    b.duckHold = hold;
    b.duck.gain.cancelScheduledValues(t);
    b.duck.gain.setTargetAtTime(1 - a, t, 0.03);
  }
  ...
  dispose() {
    ...
    this.exteriorSum.disconnect(); this.cabinLP.disconnect(); this.cabinHS.disconnect();
