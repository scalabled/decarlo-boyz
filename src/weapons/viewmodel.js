...
export class Viewmodel {
  constructor(ctx, mats) {
    ...
    // THIRD PERSON: the inherited first-person arms + weapon are drawn into
    // `viewScene`, which composites over the world with a cleared depth buffer.
    // In a third-person open-world game that puts a floating rifle and a pair of
    // gloved hands across the bottom-right quarter of every frame, including
    // every screenshot.
    //
    // `player` publishes `thirdPerson` on the system and on every `player:state`
    // payload. Hidden by default and re-evaluated each frame in update(), so a
    // future first-person mode (a cockpit view, a sniper scope) can simply flip
    // the flag back. Owned by `weapons`; gated here rather than in `player`
    // because `viewScene` membership is this subsystem's to decide.
    this.anchor.visible = false;
    this._syncViewmodelVisibility = () => {
      const p = ctx.peek('player');
      // Default to HIDDEN when player hasn't reported yet: a stray first-person
      // frame is far more damaging than a missing one.
      this.anchor.visible = p ? p.thirdPerson !== true : false;
    };
    this._syncViewmodelVisibility();
    ctx.events.on('player:state', (e) => {
      if (e && typeof e.thirdPerson === 'boolean') this.anchor.visible = !e.thirdPerson;
    });
   ...
   */
  update(dt, s) {
    // Re-evaluated every frame: `player` may not have been registered yet when
    // this was constructed, and the mode can change at runtime.
    this._syncViewmodelVisibility?.();
    const w = this.active;
    if (!w) return;
    const def = w.def;
