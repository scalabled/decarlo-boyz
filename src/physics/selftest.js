...
section('Hitbox colliders');
{
  ...
  // The round penetrates the head and then strikes the torso behind it, so only
  // the first damage event is asserted on.
  let dealt = null;
...
/* ---------------- ground fallback ---------------- */
section('Ground fallback (the collision-world holes)');
{
  const { GroundFallback } = await import('./ground.js');

  // A stand-in for `world`: an analytic walkable surface with a hill, a
  // valley, a steep bluff and a fine detail band.
  const H = (x, z) =>
    26 * Math.sin(x * 0.0021) * Math.cos(z * 0.0017) +
    52 / (1 + Math.exp(-(x - 300) * 0.02)) +
    0.9 * Math.sin(x * 0.084) * Math.cos(z * 0.031);
  const fakeWorld = { walkableHeightAt: H, heightAt: (x, z) => H(x, z) - 0.7, surfaceAt: () => 'asphalt' };

  const gf = new GroundFallback();
  ok(gf.bind(fakeWorld), 'binds to world');

  // It must prefer walkableHeightAt: `heightAt` is the TERRAIN, which is sunk
  // 0.55 m under every road corridor with the kerb above that, so using it as
  // a ground reference puts every impact under the street.
  ok(Math.abs(gf.heightAt(120, -80) - H(120, -80)) < 1e-4,
    'reads the WALKABLE surface, not the sunk terrain',
    `${gf.heightAt(120, -80).toFixed(3)} vs terrain ${(H(120, -80) - 0.7).toFixed(3)}`);

  const rng = new Rng(90210);
  let maxErr = 0, sumErr = 0, n = 0, missed = 0;
  for (let i = 0; i < 2000; i++) {
    const x = rng.range(-1500, 1500);
    const z = rng.range(-1500, 1500);
    const truth = H(x, z);
    const t = gf.ray(x, truth + 250, z, 0, -1, 0, 600);
    if (t < 0) { missed++; continue; }
    const e = Math.abs(gf.hit.py - truth);
    maxErr = Math.max(maxErr, e);
    sumErr += e; n++;
  }
  ok(missed === 0, 'vertical rays never miss', `${missed} misses`);
  ok(maxErr < 0.01, 'the hit is the exact analytic height',
    `mean ${(sumErr / n).toFixed(5)} m, worst ${maxErr.toFixed(5)} m`);

  // Slanted long rays, cross-checked against a brute-force 4000-step march:
  // a ray that legitimately leaves the city without reaching the ground is
  // not a miss and must not be scored as one.
  const brute = (ox, oy, oz, dx, dy, dz, maxT) => {
    const N = 4000;
    for (let k = 1; k <= N; k++) {
      const t = (maxT * k) / N;
      const px = ox + dx * t, pz = oz + dz * t;
      if (Math.abs(px) > gf.half || Math.abs(pz) > gf.half) return -1;
      if (oy + dy * t <= H(px, pz)) return t;
    }
    return -1;
  };
  let slantMiss = 0, slantErr = 0, slantN = 0, slantSkip = 0;
  for (let i = 0; i < 800; i++) {
    const x = rng.range(-900, 900), z = rng.range(-900, 900);
    const oy = H(x, z) + rng.range(30, 260);
    const yaw = rng.range(0, Math.PI * 2);
    const pitch = rng.range(-1.2, -0.12);
    const dx = Math.cos(pitch) * Math.cos(yaw);
    const dz = Math.cos(pitch) * Math.sin(yaw);
    const dy = Math.sin(pitch);
    const t = gf.ray(x, oy, z, dx, dy, dz, 1400);
    const ref = brute(x, oy, z, dx, dy, dz, 1400);
    if (ref < 0) { slantSkip++; continue; }
    if (t < 0) { slantMiss++; continue; }
    slantErr = Math.max(slantErr, Math.abs(gf.hit.py - H(gf.hit.px, gf.hit.pz)));
    slantN++;
  }
  ok(slantMiss === 0, 'slanted long rays agree with a brute-force march',
    `${slantMiss} missed of ${slantN + slantMiss} that should hit (${slantSkip} left the map)`);
  ok(slantErr < 0.02, 'slanted hits land on the surface', `worst ${slantErr.toFixed(5)} m`);

  ok(gf.ray(0, H(0, 0) - 8, 0, 0, -1, 0, 100) < 0, 'a ray starting underground reports no hit');
  ok(gf.ray(0, H(0, 0) + 2, 0, 0, 1, 0, 3000) < 0, 'a shot at the sky costs no march');
  ok(!Number.isFinite(gf.heightAt(9000, 0)), 'no ground invented outside the city footprint');

  // END TO END: a streamed collision patch on top, exactly the geometry
  // `world` keeps resident. Inside it a real triangle must win; outside it the
  // ray must still find the ground rather than reporting empty space.
  const phys2 = new PhysicsSystem();
  phys2.ctx = {
    events: new EventBus(), scene: null, camera: null, time: { alpha: 0 }, rng: new Rng(7),
    peek: (id) => (id === 'world' ? fakeWorld : null),
  };
  phys2._groundStarted = phys2.ground.bind(fakeWorld);
  const FINE = 320, FSTEP = 8, fn = (FINE * 2) / FSTEP + 1;
  const fine = new Float32Array((fn - 1) * (fn - 1) * 2 * 9);
  let w2 = 0;
  for (let j = 0; j < fn - 1; j++) {
    for (let i = 0; i < fn - 1; i++) {
      const x0 = -FINE + i * FSTEP, x1 = x0 + FSTEP;
      const z0 = -FINE + j * FSTEP, z1 = z0 + FSTEP;
      const y00 = H(x0, z0), y01 = H(x0, z1), y10 = H(x1, z0), y11 = H(x1, z1);
      fine[w2++] = x0; fine[w2++] = y00; fine[w2++] = z0;
      fine[w2++] = x0; fine[w2++] = y01; fine[w2++] = z1;
      fine[w2++] = x1; fine[w2++] = y11; fine[w2++] = z1;
      fine[w2++] = x0; fine[w2++] = y00; fine[w2++] = z0;
      fine[w2++] = x1; fine[w2++] = y11; fine[w2++] = z1;
      fine[w2++] = x1; fine[w2++] = y10; fine[w2++] = z0;
    }
  }
  phys2.staticWorld.addTriangles(fine, (fn - 1) * (fn - 1) * 2, 'asphalt', LAYER.STATIC, 'test:streamed-patch');
  phys2.rebuildStatic();

  let inNear = 0, fromMesh = 0, inFar = 0, farErr = 0;
  for (let i = 0; i < 600; i++) {
    const nx = rng.range(-260, 260), nz = rng.range(-260, 260);
    const hn = phys2.raycast(nx, H(nx, nz) + 120, nz, 0, -1, 0, 300, MASK.BULLET);
    if (hn.hit && Math.abs(hn.point.y - H(nx, nz)) < 0.25) inNear++;
    if (hn.triangle >= 0) fromMesh++;
    // Far outside the patch — this is where combat used to stop.
    let fx = rng.range(-1400, 1400), fz = rng.range(-1400, 1400);
    if (Math.abs(fx) < 420 && Math.abs(fz) < 420) fx += 800;
    const hf2 = phys2.raycast(fx, H(fx, fz) + 180, fz, 0, -1, 0, 400, MASK.BULLET);
    if (hf2.hit) { inFar++; farErr = Math.max(farErr, Math.abs(hf2.point.y - H(fx, fz))); }
  }
  ok(inNear === 600, 'inside the streamed patch the real collider still wins',
    `${inNear}/600 correct, ${fromMesh}/600 came from a triangle`);
  ok(fromMesh === 600, 'and the fallback never pre-empts a real triangle', `${fromMesh}/600`);
  ok(inFar === 600, 'OUTSIDE it a bullet ray still finds the ground', `${inFar}/600`);
  ok(farErr < 0.02, 'and lands on the visible surface', `worst ${farErr.toFixed(4)} m`);
  ok(phys2.worldHolesAfterReady === 0, 'no silent holes reported', `${phys2.worldHolesAfterReady}`);

  const gh = phys2.groundHeight(-980, 1120);
  ok(Number.isFinite(gh) && Math.abs(gh - H(-980, 1120)) < 0.02,
    'groundHeight() works a kilometre from the camera', `${gh.toFixed(3)} vs ${H(-980, 1120).toFixed(3)}`);

  // An explosion a kilometre away must find a surface to sit on, which is the
  // whole point: `explode` and `fireBullet` both route through raycast.
  const imp = phys2.fireBullet({
    origin: { x: -980, y: H(-980, 1120) + 40, z: 1120 },
    dir: { x: 0.02, y: -1, z: 0.02 }, damage: 40, maxDist: 120, mask: MASK.BULLET,
  });
  ok(imp.length > 0 && Math.abs(imp[0].point.y - H(imp[0].point.x, imp[0].point.z)) < 0.2,
    'fireBullet() impacts on the ground a kilometre from the camera',
    imp.length ? `${imp.length} impact(s), surface ${imp[0].surface}` : 'NO IMPACT');
